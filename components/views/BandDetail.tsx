...
const LISTEN_ORDER: Array<{ key: keyof AtlasArtist["listen"]; svc: string; note: string; primary?: boolean }> = [
        ...
        <div className="wos-band-hero" style={{ marginTop: 28 }}>
          <div className="wos-band-hero-art">
            ...
            <div className="micro" style={{ marginTop: 10, display: "flex", justifyContent: "space-between" }}>
              <span><em className="serif italic">{ref.title}</em></span>
              <span>[{ref.year}]</span>
            </div>
            ...
            {artist.photo?.url && (
              <div style={{ marginTop: 18 }}>
                ...
                />
                {artist.photo.credit && (
                  <div className="micro" style={{ marginTop: 6, color: "var(--ink-faint)" }}>{artist.photo.credit}</div>
                )}
              </div>
            )}
          ...
          <div>
            ...
            <h1 className="wos-band-h1" style={{ fontSize: "clamp(54px, 8vw, 104px)", lineHeight: 0.9, letterSpacing: "-0.02em" }}>
              {artist.name}<span className="italic" style={{ color: accentSoft }}>.</span>
            </h1>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "10px 32px", fontSize: 12 }}>
              ...
              <div>
                ...
                <div style={{ marginTop: 4 }}>{eraLabel(artist.era)}{eraInfo ? ` (${eraInfo.range})` : ""}</div>
              </div>
              <div>
              ...
              <div>
                ...
                <div style={{ marginTop: 4, fontFamily: "var(--font-jetbrains-mono)", letterSpacing: 1 }}>
                  {Array.from({ length: 10 }).map((_, i) => (
                    <span key={i} style={{ color: i < artist.intensity ? "var(--ink)" : "var(--rule)" }}>{i < artist.intensity ? "█" : "░"}</span>
                  ))}
                  <span className="micro" style={{ marginLeft: 8 }}>{artist.intensity} / 10</span>
                </div>
              </div>
              <div style={{ gridColumn: "1 / -1" }}>
                ...
                <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 4 }}>
                  {artist.moods.map((m) => {
                    ...
                    if (!mc) return null;
                    return (
                      <span key={m} className="micro" style={{ background: `hsl(${mc.hue}, 55%, 86%)`, color: `hsl(${mc.hue}, 55%, 28%)`, padding: "4px 8px", letterSpacing: "0.08em", textTransform: "uppercase" }}>{mc.label}</span>
                    );
                  })}
                </div>
              </div>
        ...
        <section style={{ marginTop: 56 }}>
          ...
          <div className="wos-band-services">
            {LISTEN_ORDER.map((s) => {
              ...
              return (
                <a key={s.svc} href={url || "#"} target={url ? "_blank" : undefined} rel={url ? "noopener noreferrer" : undefined} aria-disabled={!url} style={!url ? { opacity: 0.45, pointerEvents: "none" } : undefined}>
                  ...
                  <div className="micro" style={{ marginTop: 4 }}>{url ? `${s.note} →` : "not yet linked"}</div>
                </a>
              );
            })}
          </div>
              ...
              return (
                <div key={a.slug} onClick={() => router.push(`/band/${a.slug}`)} style={{ cursor: "pointer", borderTop: "1px solid var(--rule)", paddingTop: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                    ...
                    <span className="micro" style={{ color: `hsl(${p.hue}, 55%, 38%)` }}>● {a.moods[0]?.replace(/_/g, " ")}</span>
                  </div>
                  <AlbumCover album={aRef} palette={p} />
                  <div style={{ marginTop: 10, fontSize: 13, fontWeight: 500 }}>{a.name}</div>
                  <div className="micro" style={{ marginTop: 2 }}>{aRef.title} · {aRef.year}</div>
                </div>
              );
            })}
          </div>
